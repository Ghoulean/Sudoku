...
public final class Grid4x4Validator extends AbstractValidator {

    private static final int SUBGRID_WIDTH = 4;
    private static final int SUBGRID_HEIGHT = 4;
    private static final int RIGHT_SUBGRID_OFFSET = 3;
    private static final int BOT_SUBGRID_OFFSET = 3;

    @NonNull @Getter private final TokenSetType tokenSetType;
    @NonNull @Getter private final Set<Token> tokenSet;

    /**
     * Enforces that every token is unique per 4x4 grid, where each grid is aligned on 4 tiles. On a normal 16x16 board,
     * this is the typical grid seen. On non-16x16 boards, if there is any remainder on the right and bottom edges, they
     * are ignored.
     * @param tokenSetType tokenset to use
     */
    public Grid4x4Validator(final TokenSetType tokenSetType) {
